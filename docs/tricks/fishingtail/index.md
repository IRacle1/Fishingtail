...
*Так если фишинг это просто идея на основе которой можно придумать сотни элементом, то как мы можем называть отдельный трюк - фишингом?*
...
Meh, это опять тонкий момент в нотации, так уж сложилось что в целом `fishing` - имеет целых **3** значения:
 ...
 - `index/midde/ring/pinky fishing` - та же история что и тамб фишингом, но эти трюки немного от него отличаются(что и было рассмотренно на примере индекс/тамб фишинга)
:::
...
Эта линка в `PD` конечно несовсем возможна, но в `PS` её можно выполнить легко
