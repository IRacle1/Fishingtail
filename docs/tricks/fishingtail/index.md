...
И еще записывая трюк как `PU-PD thumb fishing rev`, я имею ввиду ввиду ИМЕННО `PU-PD (thumb fishing rev)` , а не `(PU-PD thumb fishing) rev`. Хоть в данном случае другой вариант более точный, я предпочту записывать такие фишинги с рукой именно в таком виде.
:::
...
::: danger
На самом деле нет, это определение вообще ошибочно, и работает только конкретно в нашем случае на примере тамб фишинга. 
:::
